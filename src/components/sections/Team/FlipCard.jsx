...
export const FlipCard = ({ member }) => {
  ...
  return (
    <div className="group h-[400px] sm:h-[430px] md:h-[450px] w-full perspective-1000 cursor-pointer">
      <div className="flip-inner relative w-full h-full transition-transform duration-700 transform-style-3d shadow-2xl rounded-[3rem]">
        
        {/* Lado Frontal: Foto integrada y badge en la parte inferior baja */}
        <div 
          className="absolute inset-0 backface-hidden rounded-[3rem] overflow-hidden bg-[#1A1A1A] border border-white/10"
        ...
        >
          ...
          <div className="absolute inset-0 p-8 md:p-10 flex flex-col justify-end items-center text-center z-10 pb-8 md:pb-10">
            <h3 className="text-3xl md:text-4xl font-display text-white mb-2.5 drop-shadow-xl font-bold tracking-tight">
              {member.name}
            </h3>
            <div className="bg-white/15 text-white px-5 py-1.5 rounded-full text-xs uppercase tracking-widest border border-white/10 backdrop-blur-md shadow-lg font-bold">
              {member.role}
            </div>
          </div>
          
          <div 
            className="absolute top-6 right-6 md:top-8 md:right-8 p-3 rounded-full z-10"
            style={{ 
              backgroundColor: member.accentColor,
              boxShadow: `0 0 20px ${member.shadowGlow}`
            }}
          >
            <ArrowRight className={`${isAlicia ? 'text-white' : 'text-[#022E21]'} -rotate-45`} size={20} />
          </div>
        </div>

        {/* Lado Posterior: Información detallada (Flip 180) */}
        <div 
          className="absolute inset-0 backface-hidden rotate-y-180 rounded-[3rem] p-8 md:p-10 flex flex-col justify-center border"
          style={{
        ...
        >
          <h4 className={`text-2xl md:text-3xl ${isAlicia ? 'text-white' : 'text-[#022E21]'} mb-4 font-display font-bold`}>
            {member.title}
          </h4>
